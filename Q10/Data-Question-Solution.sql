...
(
	select
		*,
		case
			when temperature < 0 and lead(temperature) over() < 0 and lead(temperature,2) over() < 0 then 1
			when temperature < 0 and lag(temperature) over() < 0 and lead(temperature) over() < 0 then 1
			when temperature < 0 and lag(temperature) over() < 0 and lag(temperature,2) over() < 0 then 1
			else 0
		end as is_selected
		from weather
)
